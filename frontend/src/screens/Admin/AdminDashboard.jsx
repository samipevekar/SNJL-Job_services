...
import { getAllPostedJobsAsync, editPostedJobAsync } from '../../redux/slice/postedJobSlice';
...
const AdminDashboard = () => {
  ...
  const handleStatusChange = (status) => {
    ...
    setEditMode(null); // Reset edit mode when changing status filter
  };
  ...
  const handleSaveChanges = async (jobId, updateData) => {
    try {
      await dispatch(editPostedJobAsync({ id: jobId, updateData })).unwrap();
      setEditMode(null);
          ...
          {jobs.length === 0 ? (
            ...
            jobs.map((job) => (
              <PostedJobCard 
                ...
                onSave={(updateData) => handleSaveChanges(job._id, updateData)}
              />
            ))
          )}
        </ScrollView>
